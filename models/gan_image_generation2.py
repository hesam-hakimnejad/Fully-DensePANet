...
#_samples=50
states2=states[:,2]
states2=np.transpose(states2) 
b1=states2-np.min(states2)
u=np.max(states2)-np.min(states2)
b=b1/u
b=b-.5
states2=b/.5 
def generate_latent_points(latent_dim, n_samples):
    ...
    for i in range(n_samples):
        c=random.randint(0, 4000-latent_dim)
        b=np.asarray(states2[c:c+latent_dim])
...
# load model
model = load_model('generator_model_065.h5')
# all 0s
...
data = load('maps_512.npz')
src_images = data['arr_0']
pyplot.imshow(src_images[5].astype('uint8'),cmap='gray_r')
pyplot.show()
